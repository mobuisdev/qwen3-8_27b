...
exec systemd-run --user --scope --quiet \
  ...
  --model-path "$MODEL" --served-model-name qwen38-radixark-bf16kv \
  --host 127.0.0.1 --port 30000 --context-length 101000 \
  --mem-fraction-static 0.91 --chunked-prefill-size 2048 \
  --attention-backend flashinfer --disable-flashinfer-autotune \
  --kv-cache-dtype bfloat16 --max-running-requests 1 --cpu-offload-gb 0 \
